...
const RequiredInputComponent = (props: { title: string, type: string, borderError: string, placeholder: string, value: string, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, maxLength: number}) => {
    return (
        <>
            ...
            <input required type={props.type} className={"w-full mt-3 mb-4 sm:min-h-16 min-h-12 jura text-3xl rounded-lg pl-3 " + props.borderError} placeholder={props.placeholder} value={props.value} onChange={props.onChange} maxLength={props.maxLength}/>
        </>
    )
}
